...
import { getPoolContract, getALMManagerContract } from "../utils/contracts";
import { PoolData, ALMState, MonitoringConfig } from "../types/index";
...
export class PoolMonitor extends EventEmitter {
  ...
  constructor(config: MonitoringConfig) {
    ...
    this.almContract = getALMManagerContract(
      config.almManagerAddress,
      this.provider
    );
  ...
  private shouldRebalance(poolData: PoolData, almState: ALMState): boolean {
    ...
    const timeSinceLastRebalance =
      Date.now() / 1000 - Number(almState.lastRebalanceTimestamp);
    const minTimeElapsed =
      timeSinceLastRebalance >= this.config.minRebalanceIntervalSeconds;
